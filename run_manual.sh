#!/bin/bash
# Manual Docker commands – fallback if Nextflow fails
...
mkdir -p "$OUTDIR_BASE"/{deepvariant,clair3,renamed_vcfs,benchmark}

# ------------------- DeepVariant -------------------
echo "Running DeepVariant (make_examples)..."
...
echo "Running DeepVariant (call_variants)..."
docker run --rm -u $(id -u):$(id -g) -v $(pwd):/data \
  google/deepvariant:1.6.0 \
...
echo "Running DeepVariant (postprocess_variants)..."
docker run --rm -u $(id -u):$(id -g) -v $(pwd):/data \
  google/deepvariant:1.6.0 \
